...
| Adapter id | 包装 | 默认可执行 | 单次 spawn 路径覆盖 |
|---|---|---|---|
...
默认可执行在 `$PATH` 上查找。若要把某个 adapter 指到非默认路径，在 `sessions.create` 的 `program_override` 字段传绝对路径（或在 debug 版 `lad` 上用仅测试用途的 `--test-shell-adapter` 标志）。v1 还没有从 `config.toml` 读持久化的 `adapters.*.command` —— 那是 follow-up。
...
> **v1 状态。** daemon 一侧 `adapters.discover` 与 `sessions.import` 完整接入（`(backend, external_id)` 唯一约束由迁移 `0003_session_external_reference.sql` 强制）。**TUI 导入浮层仍属于 M1.7 未完工** —— 今天按 `i` 只会翻转 TUI mock `SessionSource` 上的一个标志位，并不会调用 `sessions.import`。v1 真正要导入的话，通过 IPC socket 调这对 RPC。
...
{"jsonrpc":"2.0","id":2,"method":"sessions.import","params":{
  "backend":      "claude",
  "external_id":  "<discover 结果里的 external_id>"
}}
```
...
M1.7 导入浮层落地后，TUI 的 `i` 键会 round-trip 到同一对 RPC；在那之前它只操作本地 mock 状态。

## resume 一条导入的会话

resume 在 roadmap 上（daemon 源码里标 M2.4+），v1 **尚未接入**。落地时不会接管后端原先的 PTY（早没了）—— daemon 会用后端自己的 resume 标志拉起一个全新进程，指向你之前发现的 `external_path`。后端按手动 `claude --resume` 那样读自己的脚本文件，LazyAgents 把新 PTY 的字节并行记到自己的脚本里。
