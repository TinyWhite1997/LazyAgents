...
v1 的 Sessions tab 完成了导航、侧栏与 modal 框架，但 New-session 表单本身仍是占位 —— 在项目上按 **`n`** 会打开一个仅确认按键的 modal，尚不会拉起后端。表单、项目选择器与 live attach 在源码中标记为 "M1.7"，将随后续版本到位。
...
| `j` / `k` / 方向键 | 移动会话列表光标。 |
| `Enter` | attach 到选中的会话。 |
| `Esc` | 从当前会话 detach（它仍在跑）。 |
| `q` | 退出 `la`。会话与 daemon 都还活着。 |

**detach vs 退出：** detach 仅释放你的查看器；退出 `la` 在此基础上额外关闭 TUI 进程。两者都不会停止会话。让会话活着的是 daemon。
