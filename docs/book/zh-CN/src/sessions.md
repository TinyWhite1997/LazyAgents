...
| 状态 | 含义 | TUI 标记 | 侧栏颜色 |
|---|---|---|---|
| `starting` | PTY 已拉起，还没有输出。250 ms 静默或首个输出字节后自动晋升为 `running`。 | `●` | 蓝 |
| `running` | 正在收发输出。 | `●` | 蓝 |
| `waiting` | 距上一字节超过 2 秒；后端在空闲等你。只有 stdin 是 PTY 的（交互）会话能进入此状态。 | `⏸` | 绿 |
| `exited` | 子进程已结束。脚本仍保留。 | `·` | 绿 |
| `errored` | 预留给 adapter 层失败。当前尚未触发。 | `✗` | 红 |
...
Sessions 侧栏按**项目目录**分组。要新建一个项目（之后 `n` 才有可挂载的目标），在 Sessions 侧栏按 **`Shift+N`**——空 workspace 也可以按，这是首次启动后接入第一个项目的唯一入口。

New-project modal 只有一个字段：目录路径。

- 默认起始路径为用户 home 目录，可以从那里继续编辑。
- 开头的 `~` 会展开为 `$HOME`（Windows 上为 `%USERPROFILE%`）。
- 隐藏目录（`.git`、`.cache` 等）默认不会出现在候选里，除非前缀显式以 `.` 开头。
- 只列出**目录**，不会列出普通文件。
- modal **不会**创建目录。路径不存在时，modal 保持打开并给出明确提示，方便你修正。

modal 内的快捷键：

| 键 | 行为 |
|---|---|
| 任何可打印字符 | 追加到路径缓冲区。这里 `q`/`?`/数字/字母都是字面输入。 |
| `Backspace` | 删除最后一个字符。 |
| `Tab` / `↓` | 选中下一个候选目录。 |
| `Shift+Tab` / `↑` | 选中上一个候选目录。 |
| `→` | 把选中的候选填入路径，保持 modal 打开（进入下一级）。 |
