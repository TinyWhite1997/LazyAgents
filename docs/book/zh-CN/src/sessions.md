...
v1 的 Sessions tab 完成了导航、侧栏与所有 modal，**WEK-94（WEK-92-A2）起 New-session 表单已端到端接入**：在项目上按 **`n`** 会打开一个真表单，可以挑 backend、敲提示词、勾选 worktree，按 **`Ctrl+Enter`** 即调 daemon 的 `sessions.create`。新会话会在下一次 ~2 s 刷新里出现在侧栏。
