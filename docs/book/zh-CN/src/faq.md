...
`crons.upsert` 始终把 cron 落成 disabled —— 你必须调 `crons.set_enabled { cron_id, enabled: true }` 才会真正启用。之后改任何敏感字段（backend、schedule、prompt、args、budget —— 见 `crates/la-daemon/src/cron_security.rs` 里的 `SENSITIVE_CRON_FIELDS`）会**自动 disable** 它，下次触发前必须再调一次 `set_enabled`。daemon 还内置一个确认 token + summary helper（5 分钟 TTL、单次使用），原本意图给 `set_enabled` 加一道两步闸门 —— 它住在 `cron_security` 里，但 v1 中尚未接到 RPC 表面。脚本化访问 `crons.*` 时请按此预期来规划。
