...
### 怎么让 `la --check-update` 指向我自己的 fork 或内网镜像？

设置 `LAZYAGENTS_UPDATE_MANIFEST_URL` 为一个 GitHub-兼容的 Releases 端点（响应需带 `tag_name`、`html_url`、`prerelease` 字段）：
...
这是 fork、离线网络、内网镜像唯一的逃生通道。如果指定的 URL 访问不通或返回体格式错误，`la --check-update` 会把简短原因写到 stderr 并以 `0` 退出 —— 这个检查刻意做成非致命的。详见 [Install → 更新](install.md#更新)。
