...
### 我是 self-hosted fork / 我们网络访问不到 `api.github.com`

把 `LAZYAGENTS_UPDATE_MANIFEST_URL` 指向你自己镜像的 GitHub Releases 兼容端点即可，`la --check-update` 会改查该端点。响应只需 JSON 包含 `tag_name`、`html_url`、`prerelease` 这几个字段。完整示例见 [`docs/install.md#forks--air-gapped-networks-lazyagents_update_manifest_url`](../../../install.md#forks--air-gapped-networks-lazyagents_update_manifest_url)。
...
v1 中**唯一的保护是你必须显式调用 `crons.set_enabled`**。`crons.upsert` 对全新 id 落成 disabled，但对**一条已经 enabled 的 cron 做更新** —— 即便改 backend、schedule、prompt、args 或 budget —— 仍然保留 enabled，下次触发会用新值。daemon 的 `cron_security` 模块（`crates/la-daemon/src/cron_security.rs`）定义了 `SENSITIVE_CRON_FIELDS` 白名单与确认 token + summary helper（5 分钟 TTL、单次使用、64 KiB `MAX_PROMPT_BYTES` prompt 上限），未来会做 auto-disable + token 闸门，但**这些 helper 在 v1 dispatcher 中尚未接入**。脚本化做风险编辑时，请先 `set_enabled { enabled: false }`。
