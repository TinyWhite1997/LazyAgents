...
**当前结论：Linux v1 RC allowed；full GA blocked on WEK-68。**

按 owner 在 WEK-46 上对 GA 范围决策（选项 B：拆分 RC）：v1 阶段先放 "Linux v1 RC"，工程门已绿即可放行；真实 Alpha/Beta cohort 指标与屏幕阅读器实测作为 GA 前 follow-up，由 [WEK-68](https://multica.dev/issue/cd4a35b7-de2c-446f-999b-25ea3c543303) 统一招募与回填，闭环后才宣称 full GA。

Linux 自动化回归、TUI 渲染烟雾、键盘路径和 WCAG 对比度单测均通过，工程门满足 RC 门槛；外部 Alpha/Beta 用户招募和真实屏幕阅读器（NVDA / JAWS / VoiceOver / Orca）会话在 RC 放出后由 WEK-68 接手——这些项目在 RC 阶段标"待 WEK-68"，不再作为 RC 阻断项。

| 指标 | 目标 | 本次 Linux 结果 | RC 状态 | GA 状态 |
|---|---:|---:|---|---|
| 自动化测试崩溃率 | < 1% | 0 / 583 = 0% | 通过 | 通过 |
| 自动化测试错误率 | < 5% | 0 / 583 = 0% | 通过 | 通过 |
| Story 1 键盘完成 | 需通过 | 代码级键盘路径覆盖；本地可达 | 通过 | 部分通过（待 WEK-68 补真实用户） |
| Alpha CSAT | >= 4 / 5 | 未采集真实用户评分 | 不阻断 RC | 待 WEK-68 |
| Beta CSAT | >= 4 / 5 | 未开启 100 人 Beta | 不阻断 RC | 待 WEK-68 |
| Onboarding 时长 | 需记录 | 未采集真实用户数据 | 不阻断 RC | 待 WEK-68 |
| Story 1 屏幕阅读器完成 | >= 1 名用户 | 未完成真实 AT 会话 | 不阻断 RC | 待 WEK-68 |

### WEK-68 真实 cohort 采集状态（2026-06-04）

WEK-68 要求记录真实 Alpha（团队 + 10 名终端重度用户）与 Beta（100 名公开招募用户）的 crash / error / CSAT / onboarding 数据。当前仓库与 issue 历史没有提供任何可核验的真实参与者记录、反馈表导出、遥测汇总或用户访谈 transcript；因此本报告只能记录“采集尚未完成”，不能把自动化测试、模拟数据或本地代理执行结果替代为真实 cohort 指标。

| Cohort | 验收目标 | 已核验真实样本 | 当前结论 | 阻断原因 |
|---|---:|---:|---|---|
| Alpha | 团队 + 10 名终端重度用户完成核心路径 | 0 | 未达标 | 缺少真实用户名单、执行记录、崩溃/错误/CSAT/onboarding 原始数据 |
| Beta | 100 名公开招募用户数据口径 | 0 | 未达标 | 未见公开招募入口、参与者记录或 Beta 指标导出 |

真实采集完成前，GA gate 仍必须保持关闭。
...
本次仅完成 Linux 本地自动化与代码级键盘验证，不能替代 10 名外部/团队用户的 Alpha 反馈。Alpha 采集台账应按参与者逐行记录，至少包含以下字段：

| 字段 | 说明 |
|---|---|
| cohort | 固定为 `alpha` |
| participant_id | 匿名 ID，例如 `alpha-001`；不要写入姓名、邮箱或访问 token |
| 用户类型 | 团队成员 / 终端重度用户 / 辅助技术用户 |
| 环境 | OS、终端、shell、字体、是否 tmux/zellij |
| 后端 | Claude / Codex / OpenCode 安装与鉴权状态 |
| Story 1 完成时间 | 从启动 TUI 到打开一个会话，单位秒 |
| Onboarding 时长 | 从安装/首次启动到能完成核心路径，单位秒 |
| 崩溃 | 是否 panic、异常退出或产生 crash 文件；记录 `0/1` |
| 错误 | 是否出现阻断或不可恢复错误；记录 `0/1`，并保留错误摘要 |
| CSAT | 1-5 分 |
| 辅助技术 | 键盘-only、屏幕阅读器、放大、高对比 |
| 完成结果 | completed / failed / abandoned |

Alpha 汇总公式：

- `crash_rate = crash_count / completed_or_attempted_users`
- `error_rate = blocking_error_count / completed_or_attempted_users`
- `csat_average = sum(csat) / csat_response_count`
- `onboarding_p50/p90 = onboarding_seconds` 的第 50 / 90 百分位

当前 Alpha 记录表：

| cohort | target_users | verified_users | completed_users | crash_rate | error_rate | csat_average | onboarding_p50 | onboarding_p90 | 状态 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---|
| alpha | >= 10 | 0 | 0 | 未采集 | 未采集 | 未采集 | 未采集 | 未采集 | 阻断 |
...
Beta 采集台账沿用 Alpha 字段，并额外记录公开招募来源、版本号、安装包渠道、反馈提交时间和是否为重复提交。当前 Beta 记录表：

| cohort | target_users | verified_users | completed_users | crash_rate | error_rate | csat_average | onboarding_p50 | onboarding_p90 | 状态 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---|
| beta | >= 100 | 0 | 0 | 未采集 | 未采集 | 未采集 | 未采集 | 未采集 | 阻断 |

### 外部采集执行清单

1. 发布 Alpha 表单，限定匿名 ID 与非敏感环境字段，避免收集 token、路径中的个人信息或完整 transcript。
2. 招募并排期团队成员 + 10 名终端重度用户；至少包含 1 名键盘 + 屏幕阅读器用户。
3. 每名用户执行 Story 1：安装/启动 LazyAgents TUI，定位项目组，打开一个会话，记录 onboarding 秒数、完成秒数、错误和 CSAT。
4. Alpha 达标后再开放 100 人 Beta；Beta 使用同一指标口径，额外记录版本号和招募渠道。
5. 将原始匿名台账汇总为上方 Alpha / Beta 表；只有 `crash_rate < 1%`、`error_rate < 5%`、`csat_average >= 4/5` 且 onboarding 分位数已记录时，才能把状态改为通过。
...
**Linux v1 RC 放行。** 工程门已全部满足（自动化崩溃率 / 错误率 / 二进制大小 / fmt / build / acceptance flake 修复全绿，详见 [WEK-46](https://multica.dev/issue/b49a429b-8d71-487a-b396-40b07673f09a) 工程门 sign-off），按 owner 在 WEK-46 上的范围决策（选项 B），先放 Linux v1 RC。

**full GA 仍阻塞于 [WEK-68](https://multica.dev/issue/cd4a35b7-de2c-446f-999b-25ea3c543303)。** RC 放出后必须由 WEK-68 完成下列真实 cohort/AT 闭环，本报告才能把结论改为 "CONFORMS / GA allowed"：

- Alpha：>= 10 名终端重度用户完成核心路径，crash < 1%，error < 5%，CSAT >= 4/5。
- AT：>= 1 名键盘 + 屏幕阅读器用户独立完成 Story 1。
- Beta：100 人公开 Beta 指标继续满足 crash / error / CSAT 门槛。

WEK-68 完成后由该 issue assignee 回填本报告 §1 与 §7 的 GA 结论，并把 GA 阻塞权一起 close。
